...
# build-image.sh
#
# Build Docker images from directory, and make sure a container is
...
###############################################################################
# Image Name: basename of the repo directory
###############################################################################
...
SHA=$(find . -type f -not -path '*/.git/*' -print0 |    
    sort -z |
    xargs -0 sha256sum |
    sha256sum |
    awk '{print $1}')    
...
if docker image inspect "$IMAGE_HASH" >/dev/null 2>&1; then
    echo
    echo "Already built:"
    echo "  $IMAGE_HASH"
